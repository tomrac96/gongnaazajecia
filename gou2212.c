        ...
        for(int j=0;j<SZEROKOSC_PLANSZY;j++)
         for(int i=0;i<WYSOKOSC_PLANSZY ;i++){
          if(teatr_wojenny.plain[i][j]==bity){
            if(i-1>=0 && teatr_wojenny.plain[i-1][j]==bity && teatr_wojenny.plain[i-1][j]!=zbadane.plain[i-1][j]){ //sprawdza czy ma sasiadow przyjaciol
                 zbadane.plain[i][j]=teatr_wojenny.plain[i][j];
                bicie_po_sznurku(teatr_wojenny,&niby_zbite,zbadane,bity,bijacy,i-1,j);
                if(teatr_wojenny.bezpieczenstwo==BEZP)
                 sojuz++;    }
             if(j-1>=0 && teatr_wojenny.plain[i][j-1]==bity && teatr_wojenny.plain[i-1][j]!=zbadane.plain[i-1][j]){
                 zbadane.plain[i][j]=teatr_wojenny.plain[i][j];
             ...
             if(i+1<=SZEROKOSC_PLANSZY && teatr_wojenny.plain[i+1][j]==bity && teatr_wojenny.plain[i-1][j]!=zbadane.plain[i-1][j]){
                ...
                if(teatr_wojenny.bezpieczenstwo==BEZP)
                 sojuz++;    }
             if(j+1<=WYSOKOSC_PLANSZY && teatr_wojenny.plain[i][j+1]==bity && teatr_wojenny.plain[i-1][j]!=zbadane.plain[i-1][j]){
                 zbadane.plain[i][j]=teatr_wojenny.plain[i][j];
                teatr_wojenny=bicie_po_sznurku(teatr_wojenny,&niby_zbite,zbadane,bity,bijacy,i,j+1);
                if(teatr_wojenny.bezpieczenstwo==BEZP)
                 sojuz++;    }
